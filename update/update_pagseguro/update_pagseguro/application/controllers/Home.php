...
class Home extends CI_Controller {
    ...
    public function envia_pagseguro() {
        $page_data['page_name'] = "envia_pagseguro";
        //$page_data['page_title'] = site_phrase('home');
        $this->load->view('frontend/'.get_frontend_settings('theme').'/envia_pagseguro', $page_data);
    }
    
    public function retorno_pagseguro() {
        $page_data['page_name'] = "retorno_pagseguro";
        //$page_data['page_title'] = site_phrase('home');
        $this->load->view('frontend/'.get_frontend_settings('theme').'/retorno_pagseguro', $page_data);
    }
    ...
    public function paypal_payment($user_id = "", $amount_paid = "", $paymentID = "", $paymentToken = "", $payerID = "", $payment_request_mobile = "") {
        ...
        // SHOW PAGSEGURO CHECKOUT PAGE
    public function pagseguro_checkout($payment_request = "only_for_mobile") {
        if ($this->session->userdata('user_login') != 1 && $payment_request != 'true')
        redirect('home', 'refresh');
        ...
        $page_data['user_details']    = $this->user_model->get_user($this->session->userdata('user_id'))->row_array();
        $page_data['amount_to_pay']   = $total_price_of_checking_out;
        $this->load->view('frontend/'.get_frontend_settings('theme').'/pagseguro_checkout', $page_data);
    }
