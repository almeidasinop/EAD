...
class Crud_model extends CI_Model
{
    ...
    public function get_courses_pagseguro($param1 = "")
    {
        if ($param1 != "") {
            $this->db->where('id', $param1);
        }
    ...
    {
        return $this->db->get_where('users', array('is_instructor' => 0));
    }
    ...
    {
        // update paypal keys
        $pagseguro_info = array();
        $pagseguro['active'] = $this->input->post('pagseguro_active');
        $pagseguro['mode'] = $this->input->post('pagseguro_mode');
        $pagseguro['email'] = $this->input->post('pagseguro_email');
        $pagseguro['token'] = $this->input->post('pagseguro_token');

        array_push($pagseguro_info, $pagseguro);

        $data['value']    =   json_encode($pagseguro_info);
        $this->db->where('key', 'pagseguro');
        $this->db->update('settings', $data);
    }
    ...
    public function add_course($param1 = "")
    {
        ...
        $data['urlpg'] = $this->input->post('urlpg');
    ...
    public function add_lesson()
    {
        ...
        public function edit_lesson($lesson_id)
        {
            ...
            function curl_request($code = '')
            {
                ...
                public function filter_transactions_for_backend($id_user, $id_courses, $status)
                {
                    if ($id_user != "all") {
                    ...
                    if ($id_courses != "all") {
                        $this->db->like('id_courses', $id_courses);
                    }

                    if ($status != "all") {
                        $this->db->where('status', $status);
                    }
                    return $this->db->get('pagseguro_transaction')->result_array();
                }
       ...
       public function update_transactions($transaction_id, $type = "")
    {
        $transactions_details = $this->get_transactions_by_id($transaction_id)->row_array();
    ...
    public function change_transactions_status($transaction = "", $transaction_id = "")
    {
        if ($status == '10') {
            if ($this->session->userdata('admin_login') != true) {
                redirect(site_url('login'), 'refresh');
        ...
        $this->db->update('pagseguro_transaction', $updater);
    }

    public function get_transactions_thumbnail_url($transaction_id, $type = 'transaction_thumbnail')
    {
        // Course media placeholder is coming from the theme config file. Which has all the placehoder for different images. Choose like course type.
        $transactions_media_placeholders = themeConfiguration(get_frontend_settings('theme'), 'transactions_media_placeholders');
        // if (file_exists('uploads/thumbnails/course_thumbnails/'.$type.'_'.get_frontend_settings('theme').'_'.$course_id.'.jpg')){
        //     return base_url().'uploads/thumbnails/course_thumbnails/'.$type.'_'.get_frontend_settings('theme').'_'.$course_id.'.jpg';
        // } elseif(file_exists('uploads/thumbnails/course_thumbnails/'.$course_id.'.jpg')){
        //     return base_url().'uploads/thumbnails/course_thumbnails/'.$course_id.'.jpg';
        ...
        //     return $course_media_placeholders[$type.'_placeholder'];
        // }
        if (file_exists('uploads/thumbnails/transaction_thumbnails/' . $type . '_' . get_frontend_settings('theme') . '_' . $course_id . '.jpg')) {
            return base_url() . 'uploads/thumbnails/transaction_thumbnails/' . $type . '_' . get_frontend_settings('theme') . '_' . $course_id . '.jpg';
        } else {
            return base_url() . $transactions_media_placeholders[$type . '_placeholder'];
        }
    }

    public function get_transactions_by_id($transaction_id = "")
    {
        return $this->db->get_where('pagseguro_transaction', array('id' => $transaction_id));
    }

    public function get_transactions_by_user_id($param1 = "")
    {
        $courses['aguardando'] = $this->db->get_where('pagseguro_transaction', array('id_user' => $param1, 'status' => '1'));
        $courses['concluido'] = $this->db->get_where('pagseguro_transaction', array('user_id' => $param1, 'status' => '3'));
        $courses['cancelado'] = $this->db->get_where('pagseguro_transaction', array('user_id' => $param1, 'status' => '7'));
        return $courses;
    }

    public function get_status_wise_transactions($status = "")
    {
        if ($status != "") {
            $transactions = $this->db->get_where('pagseguro_transaction', array('status' => $status));
        } else {
            $transactions['aguardando'] = $this->db->get_where('pagseguro_transaction', array('status' => '1'));
            $transactions['concluido'] = $this->db->get_where('pagseguro_transaction', array('status' => '3'));
            $transactions['cancelado'] = $this->db->get_where('pagseguro_transaction', array('status' => '7'));
        }
        return $transactions;
    }

    
    public function get_status_wise_transactions_for_instructor($status = "")
    {
        if ($status != "") {
            $this->db->where('status', $status);
            $this->db->where('id_user', $this->session->userdata('user_id'));
            $transactions = $this->db->get('pagseguro_transaction');
        } else {
            $this->db->where('status', 'aguardando');
            $this->db->where('id_user', $this->session->userdata('user_id'));
            $transactions['aguardando'] = $this->db->get('pagseguro_transaction');

            $this->db->where('id_user', $this->session->userdata('user_id'));
            $this->db->where('status', 'concluido');
            $courses['concluido'] = $this->db->get('pagseguro_transaction');

            $this->db->where('status', 'cancelado');
            $this->db->where('id_user', $this->session->userdata('user_id'));
            $courses['cancelado'] = $this->db->get_where('pagseguro_transaction');
        }
        return $transactions;
    }

    public function get_instructor_wise_transactions($instructor_id = "", $return_as = "")
    {
        $transactions = $this->db->get_where('pagseguro_transaction', array('user_id' => $instructor_id));
        if ($return_as == 'simple_array') {
            $array = array();
            foreach ($transactions->result_array() as $transaction) {
                if (!in_array($transaction['id'], $array)) {
                    array_push($array, $transaction['id']);
                }
            }
            return $array;
        } else {
            return $transactions;
                ...
                public function add_quiz($course_id = "")
                {
                    ...
                    // Adding pagseguro functionalities
                public function add_pagseguro_transaction($id_user = "", $id_courses =  "", $amount = "", $status = "")
                {
                    ...
                    $this->db->insert('pagseguro_transaction', $data);
                    $id_transaction_last = $this->db->insert_id();
                    return $id_transaction_last;
                }
    
                public function add_payout($id_user = "", $amount = "") {
                            $data['user_id']        = $id_user;
                            $data['payment_type']   = 'pagseguro';
                            $data['amount']         = $amount;
                            $data['date_added']     = strtotime(date('D, d M Y'));
                            $data['status']         = 1;
                ...
                public function add_enrol($id_user = "", $course_id = "") {
                            $data['user_id']     = $id_user;
                            $data['course_id']   = $course_id;
                            $data['date_added']  = strtotime(date('D, d M Y'));
                            $this->db->insert('enrol', $data);
                ...
                public function add_payment($id_user, $course_id, $method)
                {
                    $data['user_id'] = $id_user;
                    $data['course_id'] = $course_id;
                    $data['payment_type'] = $method;
                 ...
                 public function pagseguro_update_status($id = "", $status =  "")
                {
                    $data['status'] = $status;
                    $this->db->where('id', $id);
                    $this->db->update('pagseguro_transaction', $data);
                }
