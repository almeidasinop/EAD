...
class Admin extends CI_Controller {
    ...
    public function payment_settings($param1 = "") {
        ...
        if ($param1 == 'pagseguro_settings') {
            $this->crud_model->update_pagseguro_settings();
            redirect(site_url('admin/payment_settings'), 'refresh');
        }
    ...
    public function pagseguro() {
        ...
        $page_data['selected_id_user']       = isset($_GET['id_user']) ? $_GET['id_user'] : "all";
        $page_data['selected_id_courses']    = isset($_GET['id_courses']) ? $_GET['id_courses'] : "all";
        $page_data['selected_status']        = isset($_GET['status']) ? $_GET['status'] : "all";

        // pagseguro_transaction query is used for deciding if there is any transaction or not. Check the view you will get it
        $page_data['transactions']              = $this->crud_model->filter_transactions_for_backend($page_data['selected_id_user'], $page_data['selected_id_courses'], $page_data['selected_status']);
        $page_data['status_wise_transactions']    = $this->crud_model->get_status_wise_transactions();
        $page_data['id_user']            = $this->crud_model->get_users_all()->result_array();
        $page_data['courses']             = $this->crud_model->get_courses_pagseguro();
        $page_data['page_name']              = 'pagseguro_transaction';
        $page_data['page_title']             = get_phrase('active_courses');
        
        $page_data['page_name'] = 'pagseguro';
        $page_data['page_title'] = get_phrase('pagseguro');
        $this->load->view('backend/index', $page_data);
    }
    ...
    public function get_transactions() {
        ...
        $transactions = array();
        // Filter portion
        $filter_data['selected_id_user']   = $this->input->post('selected_id_user');
        $filter_data['selected_id_courses'] = $this->input->post('selected_id_courses');
        ...
        $totalData = $this->lazyload->count_all_transactions($filter_data);
        $totalFiltered = $totalData;

        // This block of code is handling the search event of datatable
        if(empty($this->input->post('search')['value'])) {
            $transactions = $this->lazyload->transactions($limit, $start, $order, $dir, $filter_data);
        }
        else {
            $search = $this->input->post('search')['value'];
            $transactions =  $this->lazyload->transactions_search($limit, $start, $search, $order, $dir, $filter_data);
            $totalFiltered = $this->lazyload->transactions_search_count($search);
        }
        ...
        if(!empty($transactions)) {
            foreach ($transactions as $key => $row) {
                $user_details = $this->user_model->get_all_user($row->user_id)->row_array();
                ...
                $delete_transactions_url = "confirm_modal('".site_url('admin/course_actions/delete/'.$row->id)."')";
