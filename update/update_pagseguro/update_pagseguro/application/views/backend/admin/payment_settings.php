...
$pagseguro_settings = $this->db->get_where('settings', array('key' => 'pagseguro'))->row()->value;
$pagseguro = json_decode($pagseguro_settings);
?>
<!-- start page title -->
<div class="row ">
    <div class="col-xl-12">
...
<div class="row">
    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-body">
                <h4 class="header-title"><p><?php echo get_phrase('setup_pagseguro_settings'); ?></p></h4>
                <form class="" action="<?php echo site_url('admin/payment_settings/pagseguro_settings'); ?>" method="post" enctype="multipart/form-data">
                    <div class="form-group">
                        <label><?php echo get_phrase('active'); ?></label>
                        <select class="form-control select2" data-toggle="select2" id = "pagseguro_active" name="pagseguro_active">
                            <option value="0" <?php if ($pagseguro[0]->active == 0) echo 'selected';?>> <?php echo get_phrase('no');?></option>
                            <option value="1" <?php if ($pagseguro[0]->active == 1) echo 'selected';?>> <?php echo get_phrase('yes');?></option>
                        </select>
                    </div>

                    <div class="form-group">
                        <label><?php echo get_phrase('mode'); ?></label>
                        <select class="form-control select2" data-toggle="select2" id = "pagseguro_mode" name="pagseguro_mode">
                            <option value="sandbox" <?php if ($pagseguro[0]->mode == 'sandbox') echo 'selected';?>> <?php echo get_phrase('sandbox');?></option>
                            <option value="production" <?php if ($pagseguro[0]->mode == 'production') echo 'selected';?>> <?php echo get_phrase('production');?></option>
                        </select>
                    </div>
                <div class="form-group">
                    <label><?php echo get_phrase('email') ?></label>
                    <input type="text" name="pagseguro_email" class="form-control" value="<?php echo $pagseguro[0]->email;?>" required />
                </div>

                <div class="form-group">
                    <label><?php echo get_phrase('token') ?></label>
                    <input type="text" name="pagseguro_token" class="form-control" value="<?php echo $pagseguro[0]->token;?>" required />
                </div>
                <div class="row justify-content-md-center">
                    <div class="form-group col-md-6">
...
<div class="col-md-5">
    <div class="alert alert-info" role="alert">
        ...
        <p><?php echo get_phrase('please_make_sure_that').' "'.get_phrase('system_currency').'", '.'"'.get_phrase('paypal_currency').'" e '.'"'.get_phrase('stripe_currency').'" '.get_phrase('are_same'); ?>.</p>
    </div>
</div>
</div>
